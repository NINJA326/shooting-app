...
function getGrowthDateValue(){
  const el=document.getElementById("growthDateView");
  const v=el ? String(el.value||"").trim() : "";
  return /^\d{4}-\d{2}-\d{2}$/.test(v) ? v : today();
}
function setGrowthDateDefault(force=false){
  const el=document.getElementById("growthDateView");
  if(!el)return;
  if(force || !/^\d{4}-\d{2}-\d{2}$/.test(String(el.value||""))){
    el.value=today();
  }
}
function formatGrowthDateDisplay(dateIso){
  const s=String(dateIso||today());
  const m=s.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if(!m)return s;
  const d=new Date(Number(m[1]), Number(m[2])-1, Number(m[3]));
  const weeks=["日","月","火","水","木","金","土"];
  return `${m[1]}.${m[2]}.${m[3]}（${weeks[d.getDay()]}）`;
}
function growthDateTimeIso(dateIso){
  const now=new Date();
  const h=String(now.getHours()).padStart(2,"0");
  const min=String(now.getMinutes()).padStart(2,"0");
  const sec=String(now.getSeconds()).padStart(2,"0");
  return `${dateIso}T${h}:${min}:${sec}`;
}
function renderRecordDates(){
  setGrowthDateDefault(false);
...
function saveGrowthRecord(){
  ...
  const growthDate=getGrowthDateValue();
  const rec={id:safeUuid(),date:growthDate,measuredAtDisplay:formatGrowthDateDisplay(growthDate),measuredAtIso:growthDateTimeIso(growthDate),player:prof.name,category:prof.category,height:Number(height.toFixed(1)),weight:Number(weight.toFixed(1)),createdAt:new Date().toISOString(),syncAction:"create"};
  ...
  document.getElementById("growthMessage").innerHTML=`<div class="success">保存しました。${safeText(formatGrowthDateDisplay(rec.date))}<br>身長 ${rec.height}cm / 体重 ${rec.weight}kg</div>`;
...
function renderAgilityView(){renderRecordDates(); renderAgilityTypeButtons(); drawAgilityLineGraph(); renderAgilityRankings(); renderAgilityRecent();}
function renderGrowthRecent(){
  ...
  el.innerHTML=rows.length?rows.map(r=>`<div class="list-item"><b>${safeText(r.measuredAtDisplay||formatGrowthDateDisplay(r.date))}</b><br>身長：<b>${r.height}cm</b> / 体重：<b>${r.weight}kg</b></div>`).join(""):"まだ記録がありません。";
}
...
async function fetchAgilityRankings(showMessage=false){
  const st=document.getElementById("agilityRankingStatus");
  if(!getGasUrl()){
    if(st)st.innerHTML="Apps Script URLが未設定です。";
    renderAgilityRankings();
    return false;
  }
  try{
    if(st&&showMessage)st.innerHTML="順位を取得中...";
    const res=await jsonp("agilityRankings",{});
    if(!res||res.status!=="ok"){
      if(st)st.innerHTML=(res&&res.message)||"順位を取得できませんでした。Apps Scriptを最新版に更新してください。";
      return false;
    }
    agilityRankingsCache=res;
    agilityRankingsFetchedAt=Date.now();
    renderAgilityRankings();
    return true;
  }catch(e){
    if(st)st.innerHTML="順位の取得に失敗しました。Apps Script URL・デプロイを確認してください。";
    return false;
  }
}
...
showView=function(id,btn){__v90_showView(id,btn); if(id==="growthView"){syncGrowthRecordsFromSheet(false);renderGrowthView();} if(id==="agilityView"){syncAgilityRecordsFromSheet(false);renderAgilityView(); if(!agilityRankingsCache)fetchAgilityRankings(false);}};
